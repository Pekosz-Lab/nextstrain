...
def update_database(db_path, fasta_file, metadata_file):
    ...
    segment_map = {
        "1": "pb1", # note that as of 2024-08-15, Amary's genome consensus pipeline uses the NCBI IBV segment number which places PB1 as "segment 1" instead of the coniacle "segment 2". 
        "2": "pb2", # flipped. See above.
        "3": "pa",
        "4": "ha",
        "5": "np",
        "6": "na",
    ...
    for record in SeqIO.parse(fasta_file, "fasta"):
        ...
        cursor.execute(f'''
            INSERT INTO influenza_genomes (seq_id, subtype, {segment_name})
            VALUES (?, ?, ?)
            ON CONFLICT(seq_id) DO UPDATE SET {segment_name} = ?, subtype = ?
        ''', (seq_id, subtype, sequence, sequence, subtype))
    ...
    for _, row in metadata_df.iterrows():
        ...
        cursor.execute('''
            INSERT INTO influenza_genomes (seq_id, sample_id, subtype, collection_date, passage_history, study_id, sequencing_run)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ...
        ''', (seq_id, sample_id, subtype, collection_date, passage_history, study_id, sequencing_run,
              sample_id, subtype, collection_date, passage_history, study_id, sequencing_run))
