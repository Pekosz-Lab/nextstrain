...
# A convenience function that scales a palette if needed
...
get_clade_colors <- function(df, subtype) {
  ...
  clades <- df %>%
    filter(subtype == subtype, !is.na(`legacy-clade`), clade != "") %>%
...
generate_plots <- function(df, subtype_name) {
  ...
  # Clade Frequency Plot
  clade_plot <- ggplot(df_filtered, aes(x = date_run, fill = `legacy-clade`)) +
    geom_bar(position = "stack") +
